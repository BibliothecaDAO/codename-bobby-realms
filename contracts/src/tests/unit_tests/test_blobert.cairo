...
mod blobert_constructor_tests {
    ...
    use blob::tests::unit_tests::utils::{
        ...
        MERKLE_ROOTS, MINT_START_TIME, _50_ONE_OF_ONE_RECIPIENTS
    };
    ...
    fn test_constructor() {
        ...
        assert(contract_state.regular_nft_seeder.read().contract_address ==  seeder, 'wrong seeder address');
        ...
        assert(contract_state.merkle_root_tier_1_whitelist.read() ==  *MERKLE_ROOTS()[0], 'wrong tier 1 merkle root');
        assert(contract_state.merkle_root_tier_2_whitelist.read() ==  *MERKLE_ROOTS()[1], 'wrong tier 2 merkle root');
        assert(contract_state.merkle_root_tier_3_whitelist.read() ==  *MERKLE_ROOTS()[2], 'wrong tier 3 merkle root');
        assert(contract_state.merkle_root_tier_4_whitelist.read() ==  *MERKLE_ROOTS()[3], 'wrong tier 4 merkle root');
        assert(contract_state.merkle_root_tier_5_whitelist.read() ==  *MERKLE_ROOTS()[4], 'wrong tier 5 merkle root');
    }
...
mod blobert_internal_tests {
    ...
    use blob::blobert::Blobert;
    use blob::tests::unit_tests::utils::{
        SEEDER, DESCRIPTOR, MERKLE_ROOTS, MINT_START_TIME, MINTER, MINTER_RECIPIENT, _50_ONE_OF_ONE_RECIPIENTS,
    ...
    fn test_assign_custom() {
        ...
        let mut recipient_index_in_original_list: Array<u8> 
            = array![4, 23, 14, 49];
        loop {
            match recipient_index_in_original_list.pop_front() {
                Option::Some(recipient_index) => {
                    ...
                    assert(contract_state.owner_of(recipient_token_id.into()) == recipient, 'wrong owner of'); 
                    assert(
                        contract_state.custom_nft_to_supply_count.read(recipient_token_id.try_into().unwrap()) 
                            == (recipient_index.try_into().unwrap() + 1), 'wrong custom nft id to supply'
                    ); 
                },
                Option::None => {break;}
        ...
        let (mut contract_state, original_recipients) = loop {
            match recipient_index_in_original_list.pop_front() {
                Option::Some(recipient_index) => {
                    ...
                    assert(contract_state.owner_of(recipient_token_id.into()) == recipient, 'wrong owner of (1)'); 
                    assert(
                        contract_state.custom_nft_to_supply_count.read(recipient_token_id.try_into().unwrap()) 
                            == (recipient_index.try_into().unwrap() + 1 - 4), 'wrong map to supply (1)'
                    ); 
                },
        ...
        loop {
            match recipient_index_in_original_list.pop_front() {
                Option::Some(recipient_index) => {
                    ...
                    assert(contract_state.owner_of(recipient_token_id.into()) == recipient, 'wrong owner of (2)'); 
                    assert(
                        contract_state.custom_nft_to_supply_count.read(recipient_token_id.try_into().unwrap()) 
                            == (recipient_index.try_into().unwrap() + 1 + 46), 'wrong custom map to supply (2)'
                    ); 
                },
                Option::None => {break;}
            }
    ...
    fn test_mint_token_collect_fee() {
        ...
        let mut contract_state = Blobert::contract_state_for_testing();
        let token_id = 1;
        contract_state.mint_token(
            token_id, 
            caller: minter,
            recipient: minter_recipient, 
            collect_fee: true
        );
    ...
    fn test_tier_merkle_root() {
        ...
        let (response_merkle_root_tier_1, max_mint_tier_1) 
            = contract_state.tier_merkle_root(WhitelistTier::One);
        let (response_merkle_root_tier_2, max_mint_tier_2) 
            = contract_state.tier_merkle_root(WhitelistTier::Two);
        let (response_merkle_root_tier_3, max_mint_tier_3) 
            = contract_state.tier_merkle_root(WhitelistTier::Three);
        let (response_merkle_root_tier_4, max_mint_tier_4) 
            = contract_state.tier_merkle_root(WhitelistTier::Four);
        let (response_merkle_root_tier_5, max_mint_tier_5) 
            = contract_state.tier_merkle_root(WhitelistTier::Five);
    ...
    fn test_caller_won_custom_token() {
        ...
        let expected_win_count 
            = num_runs / Blobert::LOSE_CUSTOM_TOKEN_DRAW_WEIGHT.try_into().unwrap();
    ...
    fn test_set_regular_image_with_collision() {
        ...
        // generate another seed for same token id
         contract_state
                .set_regular_image(token_id.try_into().unwrap());
    ...
    fn test_set_custom_image() {
        ...
        assert!(
            contract_state.supply.read().custom_nft == 1, 
                "wrong custom token count (1)"
        );
        ...
        assert!(
            contract_state.supply.read().custom_nft == 3, 
                "wrong custom token count (3)"
        );
    ...
    fn test_ensure_whitelist_mint_period() {
        ...
        start_warp(
            CheatTarget::One(starknet::get_contract_address()), 
            mint_start_time.whitelist + 1
        );
        assert!(starknet::get_block_timestamp() > mint_start_time.whitelist , "timestamp not warpped");
        assert!(starknet::get_block_timestamp() < mint_start_time.regular , "regular should be set to higher than block ts");
    ...
    fn test_set_seeder() {
        let mut contract_state = Blobert::contract_state_for_testing();
        contract_state.set_regular_nft_seeder(
...
mod blobert_write_endpoint_tests {
    ...
    use blob::blobert::Blobert;
    use blob::tests::unit_tests::utils::{
        ERC721_NAME, ERC721_SYMBOL, OWNER, MINTER, MINTER_RECIPIENT,
        SEEDER, DESCRIPTOR, MERKLE_ROOTS, MINT_START_TIME, _50_ONE_OF_ONE_RECIPIENTS,
        deploy_fee_token, deploy_seeder, deploy_descriptor,
        create_merkle_tree
    };
    ...
    fn test_mint_regular() {
        ...
        assert(
            contract_state.erc721.balance_of(minter_recipient) == 1, 
                'wrong recipient nft balance'
        );
        ...
        assert(
            contract_state.num_regular_mints.read(minter) == 1,
            'wrong num regular mints'
        );

        assert(
            contract_state.supply.read().total_nft == 1,
            'wrong num supply'
        );
    ...
    fn test_mint_regular_max_mint() {
        ...
        contract_state
            .num_regular_mints.write(
                minter,
                Blobert::MAX_REGULAR_MINT - 1
            );
    ...
    fn test_mint_custom() {
       ...
       // allows you mint a custom token 
        ...
        let image_index = contract_state
            .custom_nft_to_supply_count.read(token_id.try_into().unwrap()) - 1;
    ...
    fn test_mint_whitelist() {
        ...
        let minter = MINTER();
        let minter_recipient = MINTER_RECIPIENT();
        let (second_tier_merkle_proof, second_tier_merkle_root) 
            = create_merkle_tree(minter);
        ...
        loop {
            ...
            let token_id = contract_state.mint_whitelist(
                minter_recipient, second_tier_merkle_proof, whitelist_tier
            );
            ...
            assert(
                contract_state.supply.read().total_nft.into() == total_supply,
                'wrong num supply'
            );
    ...
    fn test_mint_whitelist__outside_whitelist_mint_period() {
        ...
        start_prank(CheatTarget::One(this), minter);
        contract_state.mint_whitelist(
                minter_recipient, array![].span(), WhitelistTier::One
            );        
    }
    ...
    fn test_mint_whitelist__bad_merkle_proof() {
        ...
        start_prank(
            CheatTarget::One(this), 
            contract_address_const::<'not whitelisted'>()
        );
        contract_state.mint_whitelist(
            minter_recipient, second_tier_merkle_proof, whitelist_tier
        );
    ...
    fn test_update_descriptor() {
        ...
        contract_state.owner_change_descriptor(
            contract_address_const::<'new_descriptor'>()
        );

        assert(
            contract_state.descriptor.read().contract_address 
                == contract_address_const::<'new_descriptor'>(), 
                'wrong descriptor'
        );
    }
...
mod blobert_read_endpoint_tests {
    ...
    fn test_descriptor(){
        let mut contract_state = call_constructor();
        let descriptor_dispatcher 
            = IDescriptorDispatcher{
                    contract_address: contract_address_const::<1337>()
                };
        contract_state.descriptor.write(descriptor_dispatcher);
        assert(contract_state.descriptor() == descriptor_dispatcher.contract_address, 'wrong descriptor');
    }
